...
profiles = {
    ...
    'chaotic_good': {
        ...
        'negative': (lambda: np.random.choice([0,
                                               np.random.choice([50, 200])],
                                              p=[.98, .02])),
        'chance': .2
    },
    'chotic_bad': {
...
df = pd.DataFrame(data, columns=['employee_id', 'team_id',
                                 'event_date', 'positive_events',
                                 'negative_events', 'recruited'])
...
notes = (pd.DataFrame(_, columns=['employee_id', 'employee_name', 'note'])
         .assign(event_date=np.random.choice(df.event_date,
                                             size=len(_), replace=True))
         )
...
events = df[['event_date', 'employee_id',
